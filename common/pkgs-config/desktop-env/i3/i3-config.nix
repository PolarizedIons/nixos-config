...
let
  ...
  set-workspace = i: ''
    set $workspace${toStr i} ${toStr i}
  '';
  out-workspace = i: ''
    workspace $workspace${toStr i} output ${toStr (gen-monitor-at i)}
  '';
  ...
  gen-workspaces = i:
    if i < workspaces then
      (set-workspace i) + gen-workspaces (i + 1)
    else
      set-workspace i;
  gen-outputs = i:
    if monitor-count == 0 then
      ...
      (if i < workspaces then
        (out-workspace i) + gen-outputs (i + 1)
      else
        out-workspace i);
